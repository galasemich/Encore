...
Esta documentación toma una decisión pedagógica importante que vale la pena aclarar. A lo largo de este recorrido, iremos haciendo dos cosas en simultáneo: por un lado, **describiremos una solución** al problema de **desarrollar un framework minimalista** y los conceptos téoricos que existen detrás de ello y, por el otro, utilizaremos ese mismo framework para **crear un servidor sencillo**. La razón detrás de esta decisión es que, de esta manera, podemos ver los *efectos* de lo que construimos al mismo tiempo que lo desarrollamos. Podemos pensar, en términos téoricos, para qué necesitamos un enrutador, por ejemplo, y desarrollarlo, pero ver ese enrutador *en funcionamiento* hecha luz sobre nuestro trabajo, y nos permite entender de manera práctica para qué lo necesitamos y cómo funciona. Estos dos procesos irán describiéndose en paralelo, para que estos efectos sean más sencillos de visualizar. 
...
```
Este es un servidor muy simple que envía un status code de 200 (OK) y un mensaje genérico a cualquier solicitud. El paso a paso, entonces, es:
...
4️⃣ Poner el servidor a escuchar solicitudes en el puerto seleccionado.   

La función `createServer` toma como parámetro una **función callback** que a su vez usar `req` y `res` como parámetros. Esta función se ejecuta *cada vez que nuestro servidor recibe una solicitud*. Esto es fundamental porque el objeto `req` nos permite acceder a datos importantes sobre las solicitudes (como la URL o el método HTTP), como así también el objeto `res` nos permite leer información sobre la respuesta que va a dar el servidor. Veremos estos conceptos en los próximos apartados. 

💬 Para seguir pensando: ¿cómo se implementa esa "llegada" de una solicitud al servidor? Es decir, ¿cómo reconoce nuestro servidor que está llegando una solicitud, y que debe responder con lo que escribimos en el cuerpo de la función callback? Eso nos lleva directamente al corazón de la implementación de Node, y de todo JavaScript: el concepto de *evento*. En términos simples: una solicitud se agrega a una *cola de eventos* cuando llega al servidor. En ese momento, Node recibe un aviso y ejecuta el callback. 

### Observando una solicitud
...
Por ejemplo, algunos *headers* típicos que vienen en una solicitud:
...
```
Si implementáramos un enrutador en nuestro proyecto, se encargaría dar el primer paso en el manejo de las solicitudes. Así como el enrutador de internet trae datos de la empresa y los envía al hogar, y viceversa, el enrutador de nuestro servidor recibe datos (la solicitud a un endpoint) y los envía (a la función controladora correspondiente). El flujo sería así: un cliente envía una solicitud a un endpoint y, si ese endpoint está registrado en nuestro servidor, el enrutador se encargaría de llamar a la función que maneja ese endpoint. Luego esa función (que puede encontrarse como handler, controlador o función controladora, todos términos equivalentes) envía una respuesta al cliente. Si el endpoint no está registrado, se envía un error 404. 
...
Luego de las importaciones necesarias y de crear nuestro objeto `app`, ahora vamos a recurrir al método `registrarRuta` para registrar tres rutas. Siguiendo el ejemplo de una aplicación para registrar tareas, tendremos: 
...
### Definiendo funciones controladoras
En el apartado anterior pusimos como ejemplo tres funciones: `inicio`, `traerTareas` y `traerUsuarios`. Estas tres funciones son las que van a responder cuando nuestro servidor reciba solicitudes a alguno de estos tres endpoints que ya definimos en nuestro objeto `rutas`. 
...
```
Este es un método sencillo que recibe dos parámetros, la ruta y el método de la solicitud (ya veremos luego cómo hacemos para acceder a esos datos). Establecemos una condición: intentamos buscar esos datos dentro de nuestro objeto `rutas` y, si encontramos alguna coincidencia, devolvemos la función controladora asociada a la ruta. Si no la encontramos, significa que la ruta solicitada no existe, entonces devolvemos `false`.
...
3️⃣ Si la función verificadora encontró un objeto que coincida con el método y ruta pasados, devuelve la función correspondiente. En ese caso, enviamos un código de status 200 y llamamos a esa función controladora.   

4️⃣ Si no encuentra ninguna coincidencia, devuelve un error 404.   
...
>Para poder correr este código en terminal, hay que tener instalado el cliente de MySQL (se puede hacer con `mysqld --install` en terminal). Es fundamental, también, tenerlo instalado como Servicio (por lo menos en Windows) dado que, de lo contrario, no podremos conectarnos.

Antes de correr los comandos, vamos a "entrar" a MySQL. Escribimos:
```bash
mysql -u root -p
```
Nos va a pedir una contraseña, que podemos dejar en blanco simplemente pulsando Enter. Luego, copiamos el código SQL que propusimos antes. 
...
>En una implementación más robusta de un servidor, en realidad es la capa de **modelos** la que se encarga de interactuar con las bases de >datos, no los controladores. Siguiendo esta arquitectura, los controladores solo se encargan de recibir una solicitud y enviar una >respuesta, así:
>
>- Controladores --> toman como parámetros req y res. Solo reciben y envían. Llaman a servicios. 
>- Servicios --> ejecutan la lógica del negocio. Llaman a modelos.
>- Modelos --> interactúan con la base de datos. Devuelve la información a servicios. 
>
>Luego, servicios recibe la información, y se la devuelve al controlador, que termina la cadena enviando la respuesta.
>
>Sin embargo, para este servidor de prueba, sencillo, que solamente busca mostrar el framework en acción, creo suficiente correr el SQL >directamente en el controlador. 

Hecha esta aclaración, vamos ahora a crear un archivo `data.js` y vamos escribir este código:
```javascript
...
>Utilizamos `resulado[0]` porque ese es el índice del elemento que contiene la información de los registros de la base de datos. `resultado` solo tiene muchos otros metadatos que no nos interesan para enviar en una respuesta al cliente. 
...
rutaProductos.get("/", controladores.traerProductos)
rutaProductos.get("/:id", controladores.traerProductoID)
rutaProductos.post("/crear", controladores.crearProducto)
rutaProductos.delete("/:id", controladores.eliminarProducto)

export default rutaProductos
```
En el código propuesto, la variable `rutaProductos` es una instancia del objeto `Router`. Por lo que vemos en las líneas siguientes, es decir, la definición de las rutas, el objeto `Router` debe contener en su definición un método para cada verbo HTTP (en Encore, `registrarRuta()` es una función "general" porque registra todas las rutas sin hacer distinción por método HTTP; el método se pasa como argumento). 

Siguiendo con el ejemplo de Express, en el `index.js` hacemos algo así:
```javascript
app.use("/productos", rutaProductos)
```
La idea es que, con el método `use()`, todas las rutas que deinimos en `rutaProductos` se registran dentro de `/productos` (porque le estamos pasando el objeto que exportamos antes)

Ahora bien, en Encore, tanto la responsabilidad de levantar el servidor como de enrutar recaen en la clase `App`. Esto no es problemático porque Encore se trata de un proyecto minimalista, pero sí es cierto que en futuras versiones sería algo a tener en cuenta para mejorar el trabajo y hacerlo más modular, un concepto que en programación es fundamental porque hace mucho más mantenibles los programas y permite escalarlos mucho más fácilmente. ¿Cómo podríamos, entonces, implementar esto en una hipotética segunda versión de Encore?
...
```
2️⃣ Luego, definiríamos las rutas en archivos separados, por ejemplo, semánticamente, así:  
```javascript
// En usuarios.rutas.js
const rutasUsuarios = new Router()

rutasUsuarios.registrarRuta("/", traerUsuarios)
rutasUsuarios.registrarRuta("/:id", traerUsuarioID)
...
```javascript
// En tareas.rutas.js
const rutasTareas = new Router()

rutasTareas.registrarRuta("/", traerTareas)
rutasTareas.registrarRuta("/:id", traerTareaID)

export { rutasTareas }
```
La idea de la modularidad es que los métodos de enrutamiento estén definidas en el objeto `Router`, no en el objeto `App`. De esta manera, tenemos las rutas divididas por "tema", lo cual hace mucho más limpio y mantenible el programa. 
...
```
Ahora, cada vez que instanciemos la clase `App` vamos a crear un objeto para las rutas y un array para almacenar los middlewares. 

La función para registrar middlewares se vería así:
```javascript
...
```
Esta función toma como parámetro otra función (el middleware), verifica que no esté ya en la lista de middlewares registrados y, si no está, lo agrega. 
...
app.levantarServidor()
```
El middleware que aparece como argumento de `registrarMiddleware()` es el siguiente:
```javascript
...
```
Básicamente, primero guardamos en la variable `middlewareAEjecutar` el middleware que vamos a ejecutar. Imaginemos que nuestro arreglo de middlewares es el siguiente:
```javascript
const middlewares = [middlewareMetodo, otroMiddleware]
```
...
Una pregunta razonable que podríamos hacernos es: si tenemos que *parsear* el body de una request... ¿En qué formato llega al servidor? En términos simples, el body de una request llega en lo que se denomina un objeto de tipo **buffer**. Un **buffer** es un conjunto de bytes crudos que representa un string. Ahora bien, desde el servidor no podemos utilizar estos streams, porque un buffer se ve por ejemplo así:
```bash
...
Necesitamos *convertir* esta cadena de bytes crudos en algo que podamos manipular con JavaScript. Una forma práctica de hacerlo es a través de un middleware que parsee el payload de la solicitud y nos permita tener acceso a esos datos, por ejemplo, para agregar una tarea, actualizar alguna información, eliminar un recurso, etc. 
...
```
En primer lugar, desestructuramos la información que viene en el body de la solicitud. Luego envolvemos la consulta en un `try/catch` y enviamos el mensaje correspondiente según se haya podido agregar la tarea o no. 
...
```bash
curl.exe -X GET http://localhost:3001/usuario/1
```
